...
7. `202605160006_unified_execution_observability.sql`
...
The seventh migration adds the v3.1 execution unification layer: `ai_trace_events`, `trace_nodes`, `benchmark_results`, and `agent_tool_calls`. These tables preserve the v3.0 schema while making every prompt, workflow, agent, benchmark, evaluation, and deployment dry run observable through one run/trace/artifact/metric backbone.
...
- `ai_trace_events`: append-only execution event stream for every run and nested node
...
- `agent_tool_calls`: per-run agent tool invocations with latency, tokens, cost, and summaries
...
- `benchmark_results`: dataset-level benchmark outputs, correctness, hallucination, rubric, cost, and regression metrics
- `benchmark_scores`: reusable benchmark metric scores
- `trace_sessions`: root trace tree per execution
- `trace_nodes`: hierarchical LangSmith-style trace nodes and sub-runs
...
| `ai_trace_events` | Actor or workspace member read; actor inserts execution events. |
...
| `agent_tool_calls` | Actor or workspace member read; actor inserts per-run tool calls. |
...
| `benchmark_results` | Actor or workspace member read; actor inserts dataset-level benchmark results. |
| `trace_sessions` | Actor or workspace member read; actor inserts. |
| `trace_nodes` | Actor or workspace member read; actor inserts hierarchical trace nodes. |
...
- AI operations: `ai_runs_workspace_started_idx`, `ai_runs_entity_idx`, `ai_runs_parent_idx`, `ai_artifacts_workspace_idx`, `ai_metrics_workspace_scope_idx`, `ai_trace_events_workspace_created_idx`, `ai_trace_events_run_idx`, `ai_trace_events_trace_idx`
- Agents: `agents_workspace_idx`, `agent_runs_agent_idx`, `agent_tool_calls_agent_run_idx`, `agent_tool_calls_workspace_idx`
- Benchmarks: `benchmark_runs_suite_idx`, `benchmark_results_suite_idx`, `benchmark_results_workspace_task_idx`
- Traces: `trace_sessions_workspace_idx`, `trace_nodes_trace_depth_idx`, `trace_nodes_run_idx`, `trace_steps_trace_idx`, `trace_logs_trace_idx`
